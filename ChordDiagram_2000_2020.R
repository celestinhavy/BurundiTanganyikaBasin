...
cat("Loading LULC rasters for 2000 and 2010...\n")
lulc_2000 <- raster("LULC2000.tif")
lulc_2010 <- raster("LULC2010.tif")

# Image compatibility checking
if (!compareRaster(lulc_2000, lulc_2010, extent=TRUE, rowcol=TRUE, crs=TRUE, res=TRUE)) {
  stop("ERROR: The rasters do not match!")
} else cat("Raster properties match.\n")

# Values extraction and null value removal
v1 <- getValues(lulc_2000)
v2 <- getValues(lulc_2010)
...
write.csv(tm, "transition_matrix_2000_2010.csv", row.names=TRUE)
...
tiff("ChordDiagram_2000_2010.tiff", width=6, height=6, units="in", res=600, compression="lzw")
...
label_positions <- tick_positions[c(3,5,7,9)]
label_texts <- c("30%","50%","70%","90%")
...
for (i in seq_along(lulc_classes)) {
  ...
  for (j in seq_along(label_positions)) {
    ...
    circos.text(
      ...
      cex = 0.75, col = "black", family = "Arial",
      font = 2
    )
  }
}
