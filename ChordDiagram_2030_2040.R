...
cat("Loading LULC rasters for 2030 and 2040...\n")
lulc_2030 <- raster("LULC2030.tif")
lulc_2040 <- raster("LULC2040.tif")

# Image compatibility checking
if (!compareRaster(lulc_2030, lulc_2040, extent=TRUE, rowcol=TRUE, crs=TRUE, res=TRUE)) {
  stop("ERROR: The rasters do not match!")
} else cat("Raster properties match.\n")

# Values extraction and null value removal
v1 <- getValues(lulc_2030)
v2 <- getValues(lulc_2040)
...
write.csv(tm, "transition_matrix_2030_2040.csv", row.names=TRUE)
...
tiff("ChordDiagram_2030_2040.tiff", width=6, height=6, units="in", res=600, compression="lzw")
